...
function garland_admin_menu() {
	$page = add_theme_page("Color Customizer", "Color Customizer", 'edit_themes', 'custom-color', 'garland_admin_page');
...
function garland_admin_head() {
	...
	echo "<script type='text/javascript' src='" . get_stylesheet_directory_uri() . "/farbtastic-prototype.js'></script>\n";
	//echo "<script type='text/javascript' src='" . get_stylesheet_directory_uri() . "/jquery.js'></script>\n";
	echo "<link rel='stylesheet' href='" . get_stylesheet_directory_uri() . "/farbtastic.css' type='text/css' />\n";
	
	echo <<<EOHEAD
<script type="text/javascript">
	imgObj = new Image();
	imgObj.src = "$stylesheet_uri/image.php?src=preview.png&top=$_top&bottom=$_bottom&base=$_base";
	function previewLoadImage() {
		if ( imgObj.complete || 'complete' == imgObj.readyState )
			$('#preview').css({ backgroundImage: 'url("' + imgObj.src + '")' });
		else
			setTimeout("previewLoadImage();",200);
	}
	function previewUpdateDisplay() {
		var e = $('.farbtastic', '#colorpicker');
		...
		$('.color', e).css('backgroundColor', f.pack(f.HSLToRGB([f.hsl[0], 1, 0.5])));

		// Linked elements or callback
		if (typeof f.callback == 'object') {
			// Set background/foreground color
			$(f.callback).each( function(i,j) {
				if ( 'preview' == j.id ) {
					var imgsrc = $(j).css('backgroundImage');
...
<style type="text/css">
	#preview {
		width: 596px;
		height: 371px;
		background: url("$stylesheet_uri/image.php?src=preview.png&top=$_top&bottom=$_bottom&base=$_base");
		position: relative;
		margin: 0 auto;
		color: $text;
	}
...
function garland_colors() {
	return array(
		'base' => array( 'label' => 'Base', 'el' => 'body, #wrapper, .commentlist .alt', 'prop' => 'background-color', 'default' => '#0072b9', 'shift' => array( '#EDF5FA', '#ffffff' )  ),
...
<div class="updated fade"><p>Custom Colors Updated</p></div>
<?php endif; ?>
	...
	foreach ( garland_images() as $src => $image ) {
		if ( is_array($image['el']) ) {
			foreach ( $image['el'] as $k => $el ) {
					echo "\n\t$el" . ' { background: ' . garland_color( $image['color'][$k], true ) . " url('" . get_stylesheet_directory_uri() . "/image.php?src=$src";
				foreach ( (array) $image['args'][$k] as $color )
					echo "&$color=" . substr(garland_color( $color ), 1);
				echo "') {$image['post'][$k]}" . ' } ';
			}
		} else {
			echo "\n\t{$image['el']}" . ' { background: ' . garland_color( $image['color'], true ) . " url('" . get_stylesheet_directory_uri() . "/image.php?src=$src";
...
<p class="submit" style="clear: both;">
	<input type="submit" value="Save Custom Colors" />
	<input type="submit" name="reset-colors" value="Reset to Default Colors" class="delete" />
</p>
</form>
<?php }
...
add_action( 'load-presentation_page_custom-color', 'garland_save_colors' );
